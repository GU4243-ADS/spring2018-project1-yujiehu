...
In the current example I’m using all the phrases I extracted from the trigrams. And then well use words that follow each other choosing “randomly” but weighted by occurrence.
...
```
* MWS still used more "normal" and "light" words than others.

* HPL is always the author used least sentiment words in this plot. Compared to lasst plot, HPL is more frequent to use "dark" sentiment description.
...
* HPL used Fear most and he is the only author who used Negative words more than positive. He is also "mean" using joy.
...
```
Calculate sentiment scores for each sentence.Showed the highest 10 score amont all sentences
...
```
* EAP is way more positive over the setence level tham other authors.The first three sentences are:
...
* No surprising that HPL has an average negative score.

* EAP got the highest score is a little surprise because MWS is the author who used more positive and good sentiment words in *nrc* lexicon.
